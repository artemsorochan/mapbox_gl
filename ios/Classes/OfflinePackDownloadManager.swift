...
class OfflinePackDownloader {
    ...
    func download() -> Int {
        ...
        let tilePyramidRegion = regionDefinition.toTilePyramidOfflineRegion()
        storage
    ...
    private func onPackCreated(pack: OfflinePack) {
    ...
    @objc private func onPackDownloadProgress(notification: NSNotification) {
        ...
        guard let pack = notification.object as? OfflinePack,
    ...
    @objc private func onPackDownloadError(notification: NSNotification) {
        guard let pack = notification.object as? OfflinePack,
              verifyPack(pack: pack) else { return }
        let error = notification.userInfo?[OfflinePackUserInfoKey.error] as? NSError
    ...
    @objc private func onMaximumAllowedMapboxTiles(notification: NSNotification) {
        guard let pack = notification.object as? OfflinePack,
              verifyPack(pack: pack) else { return }
        let maximumCount = (notification.userInfo?[OfflinePackUserInfoKey.maximumCount]
    ...
    private func setupNotifications() {
        NotificationCenter.default.addObserver(
            ...
            name: NSNotification.Name.OfflinePackProgressChanged,
            object: nil
        )
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(onPackDownloadError(notification:)),
            name: NSNotification.Name.OfflinePackError,
            object: nil
        )
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(onMaximumAllowedMapboxTiles(notification:)),
            name: NSNotification.Name.OfflinePackMaximumMapboxTilesReached,
            object: nil
        )
    }
    ...
    private func verifyPack(pack: OfflinePack) -> Bool {
